...
import { Emprestimo, EmprestimoParcela, EmprestimoDocumento } from '@/types/emprestimo'
import {
  getParcelas,
  getDocumentos,
  regenerarParcelas,
...
export function EmprestimoDetalhe({
  ...
  useEffect(() => {
    let active = true
    ;(async () => {
      const [p, d] = await Promise.all([getParcelas(e.id), getDocumentos(e.id)])
      if (!active) return
      setParcelas(p)
      setDocumentos(d)
      setLoading(false)
      ...
      <div className="bg-[#0d0d0d] border border-white/10 rounded-2xl w-full max-w-5xl shadow-2xl my-4">
        ...
        <div className="relative border-b border-white/10 p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5">
            <Campo label={isConsorcio ? 'Administradora' : 'Credor'} value={e.credor} />
            <Campo
            ...
            <Campo label="Obra vinculada" value={obraNome} />
          </div>
        </div>
                          ...
                          {parcelas.map(p => {
                            ...
                            return (
                              <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                                ...
                                <td className="px-4 py-3 text-sm border-t border-white/[0.05] text-white/70 font-mono">{fmtDate(p.vencimento)}</td>
